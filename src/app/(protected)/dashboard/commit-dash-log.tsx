...
export function CommitDashLog() {
  ...
  return (
    <ul className="space-y-6">
      {commits.map((commit: CommitWithSummary, index: number) => (
        <li key={commit.id} className="relative flex gap-x-4">
          <div className="flex flex-col items-center">
            ...
            />
            <img
              src={commit.commitAuthorAvatar || undefined}
              alt="Commit Avatar"
          ...
          <div className="flex-auto rounded-md bg-white p-3 ring-1 ring-inset ring-gray-200 dark:bg-gray-900 dark:ring-gray-700">
            <div className="flex justify-between gap-x-4">
              ...
              >
                <span className="font-medium text-gray-900 dark:text-gray-100">{commit.commitAuthorName}</span>
            ...
            <div className="mt-1 text-xs text-gray-400">{new Date(commit.commitDate).toLocaleString()}</div>
          </div>
        </li>
      ))}
    </ul>
