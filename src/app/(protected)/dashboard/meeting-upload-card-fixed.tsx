                  ...
                  <div className="mt-6 flex justify-end space-x-3">
                    <button
                      onClick={() => {
                        ...
                        const fullText = `# ${meetingSummary.meetingTitle || 'Meeting Summary'}\n\n## Full Transcript\n${meetingSummary.transcript || 'Not available'}\n\n## Key Points\n${meetingSummary.keyPoints?.map(p => `- ${p}`).join('\n') || 'None identified'}\n\n## Action Items\n${meetingSummary.actionItems?.map(a => `- ${a}`).join('\n') || 'None identified'}\n\n## Decisions\n${meetingSummary.decisions?.map(d => `- ${d}`).join('\n') || 'None identified'}\n\n## Participants\n${meetingSummary.participants?.map(p => `- ${p}`).join('\n') || 'None identified'}`;
